...
#%%
# assignment of training cells based on set cutoff
# class_size = 100
# core_meta = cell_coords.loc[core]
# core_class_sizes = core_meta.leiden.value_counts()
# training_cells = []
# for ct in core_class_sizes.index:
#     ct_size = core_class_sizes[ct]
#     if ct_size > class_size:
#         training_cells += core_meta[core_meta.leiden==ct].sample(class_size, replace=False).index.tolist()
#     else:
#         training_cells += core_meta[core_meta.leiden==ct].index.tolist()
...
# Assignment of training cells based on rank
# training_set = mask_distance.sort_values('mask_distance')
# training_set = training_set[~training_set.index.duplicated(keep='first')]
# core = training_set[training_set.mask_distance>5].index
# core = [x for x in core if x not in interface_cells]
# training_set = training_set.sort_values('mask_distance', ascending=False)
# training_set = training_set.groupby(adata.obs.loc[training_set.index, 'leiden']).head(100)
# training_cells = training_set.index.tolist()
#%%
...
# core_probs = classifier.predict_proba(pca_model.transform(core_non_train))
# int_close_probs = classifier.predict_proba(pca_model.transform(interface_test.loc[interface_close]))
# int_distant_probs = classifier.predict_proba(pca_model.transform(interface_test.loc[interface_distant]))
...
#%%
# entropies = pd.DataFrame(
#     entropy(np.concatenate([core_probs, interfaces_probs,simulated_probs]),axis=1),
#     columns = ['entropy'])
# entropies['type'] = ['core']*len(core_probs) + ['interface']*len(interfaces_probs) + simulated_type
# p = sns.kdeplot(
...
#%%
intf = interface.copy()
intf_tx = tx_metadata[tx_metadata.cell_id.isin(intf.index)][['global_x','global_y','cell_id','gene','molecule_id']]
intf_tx.columns = ['x','y','cell_id','gene','molecule_id']
intf_tx = intf_tx.merge(intf[['neaghbor_by_centroid','mask_distance']], left_on='cell_id', right_index=True)
intf_tx['tx_geom'] = intf_tx[['x','y']].apply(lambda x: Point(x.iloc[0],x.iloc[1]),axis=1)
intf_tx['mask_geom'] = cell_coords.loc[intf_tx.neaghbor_by_centroid.values, 'Geometry'].values
intf_tx['tx_mask_distance'] = distance(intf_tx['tx_geom'], intf_tx['mask_geom'])
intf_tx['celltype'] = cell_coords.loc[intf_tx.cell_id,'leiden'].values
intf_tx['neaby_celltype'] = cell_coords.loc[intf_tx.neaghbor_by_centroid,'leiden'].values
#%%
