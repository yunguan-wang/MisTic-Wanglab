...
def load_patch(adata_w_leiden_xy: pl.DataFrame,
    ...
    tx_features = {"distance_feature": torch.tensor(tx_patch[['distance_feature']].to_numpy(), 
                                                    dtype=torch.float32, device=model_device),
                   "exp_feature": torch.tensor(tx_patch[['exp_feature']].to_numpy(), 
                                                    dtype=torch.float32, device=model_device),
                   "neighbor_exp_feature": torch.tensor(tx_patch[['neighbor_exp_feature']].to_numpy(), 
                                                    dtype=torch.float32, device=model_device)}
    tx_prior_features = {'prior_distance_feature': torch.tensor(tx_patch[['prior_distance_feature']].to_numpy(),
                                                                dtype=torch.float32, device=model_device),
                         'prior_exp_feature': torch.tensor(tx_patch[['prior_exp_feature']].to_numpy(),
                                                                dtype=torch.float32, device=model_device),
                         'prior_neighbor_exp_feature': torch.tensor(tx_patch[['prior_neighbor_exp_feature']].to_numpy(),
                                                                dtype=torch.float32, device=model_device)}
